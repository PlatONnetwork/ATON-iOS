...
open class RLPDecoder {
    ...
    func arrayTake(m: Int, n: Int, arrayIn: Array<UInt8>) -> ArraySlice<UInt8>
    {
        return arrayIn[m...n]
    }
    ...
    open func decode(_ rlp: Bytes) throws -> RLPItem {
        ...
        if sign >= 0x00 && sign <= 0x7f {
            ...
            //Same with Java version if prefix sign == 0x80 return byte[0x00] ,modify by Ned 20191103.
            if count == 0 && rlp.count > 1{
                let newRlp = rlp[1...(rlp.count - 1)]
                let tmp = Array(newRlp)
                let result = try! decode(tmp)
                var items = [RLPItem]()
                items.append(0x00)
                items.append(result)
                return .array(items)
            }else if count == 0 && rlp.count == 1{
                //RLPDecoder().decode([0xc1,0x80]) ,modify by Ned 20191103
                /*
                var items = [RLPItem]()
                items.append(0x00)
                return .array(items)
