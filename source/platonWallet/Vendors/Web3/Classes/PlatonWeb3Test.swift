...
class PlatonWeb3Test {
    ...
    func testTransfer(){
        let web3tmp = Web3(rpcURL: "http://192.168.7.184:8545")
        ...
        web3tmp.eth.getTransactionCount(address: from, block: .latest, response: {
            nonceResp in
            ...
            if signedTx != nil{
                self.handleSend(signedTx: signedTx,web3: web3tmp)
            }
    ...
    func escapingCallTest(){
        ...
        web3tmp.eth.call(call: call, block: .latest) { (resp) in
...
//        let address = EthereumAddress(hexString: "0x..")
    ...
    func escapingDeployTest(){
        ...
        web3.eth.estimateGas(call: call) { (gasresp) in
            print("\(gasresp)")
            ...
            let data = EthereumData(bytes: rawRlp!)
            let privateKey = try! EthereumPrivateKey(hexPrivateKey: "4484092b68df58d639f11d59738983e2b8b81824f3c0c759edd6773f9adadfe7")
            web3.eth.getTransactionCount(address: address, block: .latest, response: {
                nonceResp in
        ...
        web3.eth.sendRawTransaction(transaction: signedTx!, response: { (resp) in
            ...
            case .success(_):
                do{
                    ...
                    queue.async {
                        repeat{
                            web3.eth.getTransactionReceipt(transactionHash: resp.result!, response: { receptionresp in
