...
extension WithDrawViewController {
    ...
    func nextButtonCellDidHandle() {
        ...
        if walletObject.currentWallet.type == .observed {
            ...
            let transactionData = TransactionQrcode(amount: self.currentAmount.description, chainId: web3.properties.chainId, from: walletObject.currentWallet.address, to: WalletUtil.convertBech32(PlatonConfig.ContractAddress.stakingContractAddress), gasLimit: selectedGasLimit.description, gasPrice: selectedGasPrice.description, nonce: nonce.description, typ: nil, nodeId: nodeId, nodeName: self.currentNode?.name, stakingBlockNum: String(stakingBlockNum), functionType: funcType.typeValue, rk: nil)
