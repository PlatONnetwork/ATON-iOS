    ...
    public:
        ...
        void executeTransaction(uint64_t transactionId) {
            ...
            if (isConfirmed(transactionId)) {
                ...
                int result = ::callTransfer((const uint8_t*)addr.c_str(), addr.length(), tx.value);
                if (result == 0) {
        ...
        int isRevoke(uint64_t transactionId) const {
            uint64_t count = 0;
        ...
        uint64_t transactionCount = 0;
             ...
             for(size_t i = from; i < end; i++){
                 ...
                 result.append(transaction.from.toString())
                         ...
                         .append("|").append(toString(transaction.value))
                         .append("|").append(toString(transaction.time))
                         .append("|").append(transaction.data)
                         .append("|").append(toString(transaction.fee))
